...
void execute(std::shared_ptr<globals_t> g, std::shared_ptr<Parameters> parameters) {
    std::vector<std::thread> threads;
    threads.reserve(MAX_THREADS_POW2);
    std::vector<std::shared_ptr<ThreadLoop>> threadLoops = parameters->getWorkload(g, g->rngs);
    ...
    for (int i = 0; i < parameters->getNumThreads(); ++i) {
        threads.emplace_back(&ThreadLoop::run, threadLoops[i]);
    }
...
void run(std::shared_ptr<globals_t> g) {
...
void printExecutionTime(std::shared_ptr<globals_t> g) {
...
void printOutput(std::shared_ptr<globals_t> g, bool detailStats = true) {
...
template <typename T>
std::shared_ptr<T> parseJsonFile(const std::string& fileName) {
    std::ifstream fin;
    fin.open(fileName);

    nlohmann::json j = nlohmann::json::parse(fin);
    auto t = std::make_shared<T>(j);
...
int main(int argc, char** argv) {
    ...
    std::shared_ptr<BenchParameters> benchParameters = std::make_shared<BenchParameters>();
    std::shared_ptr<Parameters> prefill = nullptr;
    std::shared_ptr<Parameters> warmUp = nullptr;
    ...
    std::shared_ptr<globals_t> g = std::make_shared<globals_t>(benchParameters);
